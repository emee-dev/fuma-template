...
import { Editor, EditorContainer } from "@trythis/web-editor/editor-ui";
...
export const MdxEditorPage = (props: PageProps) => {
  const editor = usePlateEditor({
    plugins: EditorKit,
    value: (editor) =>
      editor.getApi(MarkdownPlugin).markdown.deserialize(props.source),
  });
      ...
      <SidebarInset className="grid grid-rows-[40px_1fr] gap-y-2.5 scrollbar-hide">
        <header className="sticky top-0 flex items-center h-12 gap-2 p-4 border-b bg-background shrink-0">
          <div className="*:not-first:mt-2">
            <Select defaultValue="1">
              <SelectTrigger>
                ...
                <SelectValue placeholder="eg main" className="ml-1.5" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex items-center gap-x-1.5">
            ...
            <p className="text-sm">Updated 23 minutes ago</p>
          </div>
        ...
        <div className="relative">
          <div id="modal-root" className="absolute z-50 w-[600px]"></div>

          <PlateWithFS editor={editor}>
            <EditorContainer variant="demo" className="relative">
              <Editor />
            </EditorContainer>
          </PlateWithFS>
        </div>
