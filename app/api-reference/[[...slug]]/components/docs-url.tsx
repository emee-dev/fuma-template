...
export const DocsUrl = (props: { url: string; method: string }) => {
  ...
  return (
    <div
      className="group grid grid-cols-[auto_1fr_auto] h-12 gap-x-2 mt-6 border border-input p-2.5 rounded-xl items-center cursor-pointer"
      onClick={() => {
    ...
    >
      ...
      <div className="flex md:hidden md:group-hover:flex justify-start text-muted-foreground">
        {!isCopied && <Copy className="size-4 cursor-pointer" />}
        {isCopied && <CheckCircleFilled className="size-5 cursor-pointer" />}
      </div>
    </div>
  );
};
